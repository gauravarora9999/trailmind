...
export default function AuthModal({ mode, onClose, onSubmit, onSwap }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    onSubmit(isSignup ? name : 'Traveller', email || 'user@trailmind.io');
  };

  const handleSocial = (provider) => {
    onSubmit(provider + ' User', provider.toLowerCase() + '@trailmind.io');
  };
    ...
    <div className="modal" onClick={onClose}>
      <div className="auth" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <input type="password" placeholder="&bull;&bull;&bull;&bull;&bull;&bull;&bull;&bull;" value={pw} onChange={(e) => setPw(e.target.value)} />

          <button className="btn btn-coral" type="submit" style={{ width: '100%', marginTop: '20px', justifyContent: 'center' }}>
        ...
        <div className="soc-btns">
          <button className="sbtn" onClick={() => handleSocial('Google')}>
          ...
          <button className="sbtn" onClick={() => handleSocial('Apple')}>
